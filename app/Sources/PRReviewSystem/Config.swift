...
public struct Config: Codable, Equatable, Sendable {
    ...
    /// Note: githubToken is read from GITHUB_TOKEN_PR_REVIEW_SYSTEM env var, not from config file
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)

        // Token comes from environment variable, not config file
...
public struct NotificationConfig: Codable, Equatable, Sendable {
    ...
    /// Custom sound file path (mp3/wav/aiff). Can be set via config or PR_REVIEW_SOUND_PATH env var
    ...
    public init(from decoder: Decoder) throws {
        ...
        let configSoundPath = try container.decodeIfPresent(String.self, forKey: .soundPath)
        soundPath = ProcessInfo.processInfo.environment["PR_REVIEW_SOUND_PATH"] ?? configSoundPath
    }

    public init(newCommits: Bool = true, newComments: Bool = true, sound: Bool = true, soundPath: String? = nil) {
...
enum ConfigLoader {
    ...
    private static func validate(_ config: Config) throws {
        if config.githubToken.isEmpty {
            throw ConfigError.missingRequiredField(name: "GITHUB_TOKEN_PR_REVIEW_SYSTEM environment variable")
        }
