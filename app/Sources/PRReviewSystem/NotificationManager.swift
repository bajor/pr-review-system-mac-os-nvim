...
public final class NotificationManager: NSObject, @unchecked Sendable {
    ...
    /// Custom sound file path (mp3/wav/aiff)
    ...
    private func sendNotification(content: UNMutableNotificationContent, identifier: String) {
        // Play custom sound if configured (works without notification center)
        if soundEnabled {
            playCustomSoundIfConfigured()
        }
    ...
    /// Play custom sound if a path is configured
    private func playCustomSoundIfConfigured() {
        guard let soundPath = customSoundPath else { return }

        // Expand tilde in path
        let expandedPath: String
        if soundPath.hasPrefix("~") {
            let home = FileManager.default.homeDirectoryForCurrentUser.path
            expandedPath = home + soundPath.dropFirst()
        } else {
            expandedPath = soundPath
        }

        let url = URL(fileURLWithPath: expandedPath)

        guard FileManager.default.fileExists(atPath: expandedPath) else {
            print("Custom sound file not found: \(expandedPath)")
            return
        }
